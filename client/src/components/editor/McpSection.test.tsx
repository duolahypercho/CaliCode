...
vi.mock("../../lib/extensions", () => ({
  listMcpServersWithFingerprint: vi.fn(),
  approveProjectMcp: vi.fn(),
  reloadMcp: vi.fn(),
  setMcpEnabled: vi.fn(),
}));
...
const mockList = vi.mocked(listMcpServersWithFingerprint);
const mockApprove = vi.mocked(approveProjectMcp);
const mockReload = vi.mocked(reloadMcp);
const mockSet = vi.mocked(setMcpEnabled);
const mockConfig = vi.mocked(readCoreConfig);

const listing = (servers: McpServerReport[], projectFingerprint: string | null = null) => ({
  servers,
  projectFingerprint,
});
...
describe("McpSection", () => {
  it("renders server rows and expands the namespaced tool list", async () => {
    mockList.mockResolvedValue(listing([server({}), server({ id: "broken", status: "failed", error: "spawn failed", tools: [] })]));
    render(<McpSection />);
  ...
  it("toggles a server through mcp_set_enabled and shows the fresh reports", async () => {
    mockList.mockResolvedValue(listing([server({})]));
