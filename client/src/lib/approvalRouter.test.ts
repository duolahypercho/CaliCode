...
describe("reduce is total over state × event", () => {
  ...
  const events: Array<[string, ApprovalEvent]> = [
    ...
    // A grant on a sibling card covers this one. Core answered it, so the card
    // settles approved rather than lapsing as unanswerable.
    ["Resolved:always", { kind: "Resolved", requestId: "r-1", outcome: "always-allowed" }],
  ...
  const expected: Record<string, Record<string, string>> = {
    ...
    lapsed: {
      ...
      "Resolved:always": "lapsed:core-refused",
