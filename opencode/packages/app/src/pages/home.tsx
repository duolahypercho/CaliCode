...
import { sessionHasOpenTab, tabKey, useTabs } from "@/context/tabs"
...
// The "/" route resolves to a working session instead of a launcher: it resumes
// the most recent session on cold boot (or the active tab if any are open), and
// otherwise opens a fresh draft. This module-level flag limits the historical
// resume to the first "/" resolution per app launch, so later visits — e.g.
// closing the last tab — open a fresh draft rather than reopening what was closed.
let homeBootResumed = false
...
export function NewHome() {
  ...
  // While true, "/" is deciding where to send the user (resume/draft) and the
  // launcher stays hidden behind a spinner. It flips to false only when we give
  // up and fall back to the launcher (no projects to open or draft into).
  ...
  function resumeRecentTab() {
    const target = tabs.store.find((tab) => tabKey(tab) === tabs.recentKey()) ?? tabs.store[tabs.store.length - 1]
  ...
  // Resolve "/" into a working session rather than the launcher: resume the
  // active tab if any, otherwise reopen the most recent session on cold boot,
  // otherwise open a fresh draft. The launcher only appears when there is
  // nothing to resume and no project to draft into (so a project can be added).
  createEffect(() => {
    if (!resolving() || !tabs.ready()) return
    ...
    // Wait for the server context and its session index before concluding there
    // is nothing to resume.
    if (!focusedServerCtx()) return

    if (!homeBootResumed) {
      if (sessionLoad.isLoading) return
      homeBootResumed = true
      const recent = records()[0]
      if (recent) {
        setResolving(false)
        openSession(recent.session)
        return
      }
    }
    ...
    // No user project yet — draft against the server's own working directory so
    // the composer still opens instead of the launcher.
    const path = focusedSync().data.path
    const fallbackDirectory = path?.directory || path?.worktree || path?.home
    if (fallbackDirectory) {
      setResolving(false)
      void tabs.newDraft({ server: ServerConnection.key(conn), directory: fallbackDirectory })
      return
    }

    // Nothing to resume and no directory to draft into — reveal the launcher.
    if (!sessionLoad.isLoading) setResolving(false)
  })
  ...
  return (
    <Show
      when={!resolving()}
      fallback={
        <div class="m-2 flex min-h-0 flex-1 items-center justify-center self-stretch rounded-[10px] bg-v2-background-bg-base shadow-[var(--v2-elevation-raised)]">
          <Spinner />
        </div>
      }
    >
...
function HomeSessionsEmpty(props: { onNewSession?: () => void }) {
  const language = useLanguage()
  return (
    <div class="caliber-hero flex min-h-full flex-col items-center justify-center gap-4 px-6 pb-16 text-center">
      <div class="caliber-hero-mark">CALIBER</div>
      <p class="caliber-hero-tag !mt-0">
